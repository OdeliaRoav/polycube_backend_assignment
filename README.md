...
그래서 처음부터 모든 요구사항을 하나의 구조 안에 억지로 담아내기보다는 ,

- `main` (기본 요구사항) 브랜치에서는 기본 요구사항을 충실하게 구현하고
- `feature/payment-history` (심화 요구사항) 브랜치에서는 main 구조의 한계를 보완하는 방향으로 확장하고
- `feature/Swagger-test` (테스트를 위한 개량) 브랜치에서는 구현 결과를 API 레벨에서 직접 검증하는 흐름을 추가하는 방식
...
구조적으로는 할인 정책을 단순히 if-else 로직으로 처리하기보다,
등급별 정책 클래스로 분리한 뒤 `DiscountPolicyResolver`가 회원 등급에 맞는 정책을 선택해주도록 구성했습니다.
...
`main`에서 중요하다고 생각한 것은,  
기본 요구사항 수준에서는 **결제 결과를 정확하게 저장하는 것**이었습니다.

즉, 정책의 상세 이력보다는  
결제 결과 자체를 일관되게 남기는 것에 우선순위를 두었습니다.
...
`feature/payment-history`는 **운영 환경에서 발생할 수 있는 정책 변화까지 고려한 심화 설계 브랜치**를 목표로 구현했습니다.

### 6.1 왜 설계를 바꿨는가

심화 요구사항을 보면 핵심은 단순히 할인 하나를 더 추가하는 것이 아닌,

오히려 더 중요한 문제는 아래 두 가지였다고 생각합니다.
...
### 9.3 Breakpoint를 활용한 실행 흐름 확인

단위 테스트와 결과 검증 외에도, 실제 서비스 로직이 의도한 순서대로 동작하는지 확인하기 위해  
`main` 브랜치와 `feature/payment-history` 브랜치에서 브레이크포인트를 활용해 실행 흐름을 점검했습니다.

주로 아래 지점을 확인했습니다.

- 주문 조회가 정상적으로 수행되는지
- 회원 등급 할인 및 추가 할인 계산이 의도한 순서로 적용되는지
- `Payment` 저장이 정상적으로 수행되는지
- `feature/payment-history`에서는 `AppliedDiscountHistory` 저장까지 이어지는지

이 과정을 통해 단순히 테스트 결과만 확인한 것이 아니라,  
실제 실행 시 **요청 처리 → 할인 계산 → 엔티티 저장** 흐름이 설계 의도와 일치하는지도 함께 검증했습니다.
...
기능 자체보다도 **요구사항 변화에 따라 구조를 어떻게 진화시켰는가**였다고 생각합니다. 

짧은 시간 안에 최대한 완성도 있게 구현하기 위해 많은 노력을 들였다고 생각합니다. 감사합니다!!
